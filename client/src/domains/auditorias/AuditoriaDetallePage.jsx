...
const AuditoriaDetallePage = () => {
  ...
  const cargarAuditoria = async () => {
    try {
      const response = await fetch(`/api/auditorias/${auditoriaId}?incluir_timeline=true`, {
        headers: {
  ...
  const cargarDocumentos = async () => {
    try {
      const response = await fetch(`/api/auditorias/${auditoriaId}/documentos?solo_actuales=true`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
  ...
  const avanzarEtapa = async () => {
    try {
      const response = await fetch(`/api/auditorias/${auditoriaId}/avanzar-etapa`, {
        method: 'POST',
        headers: {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          {(['CARGA_PRESENCIAL', 'CARGA_PARQUE'].includes(auditoria.estado)) && (
            <button
              onClick={() => navigate(`/auditorias/${auditoriaId}/wizard`)}
