// /server/server-chat-auth-etl-integrated.js
// Servidor integrado con Auth + Chat + ETL + Auditorías
...
// Hacer que sequelize esté disponible para todos los módulos
app.use((req, res, next) => {
  req.sequelize = sequelize;
  req.authenticateToken = authenticateToken;
  req.authorizeRoles = authorizeRoles;
...
async function initializeApp() {
  try {
    ...
    // Sincronizar modelos ETL si están disponibles
    try {
      await sequelize.sync({ alter: false });
      console.log('✅ Modelos de base de datos sincronizados');
    } catch (syncError) {
      console.log('⚠️ Error sincronizando modelos:', syncError.message);
    }
    ...
    server.listen(PORT, () => {
      console.log('🚀 ============================================');
      console.log('🎯 Portal de Auditorías - Sistema Integrado');
      console.log('🚀 ============================================');
